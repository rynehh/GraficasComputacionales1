...
public:
	...
	void animateWater(OpenGLClass* OpenGL) {
		...
		else if (yMove >= .5 && goUp) {
			goUp = false;
		}
		else if (yMove >= 0 && !goUp) {
			yMove -= 0.01;
		}
		else {
			goUp = true;
		...
		OpenGL->MatrixRotationY(matrixAux, 0.1);
		OpenGL->MatrixMultiply(worldMatriz, worldMatriz, matrixAux);
