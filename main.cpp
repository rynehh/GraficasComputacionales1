...
void ReproductorInicializaYReproduce() {
	...
	Cancion[0].Dir = "recursos/animalc.mp3";
	Cancion[1].Nombre = "Inicio 1";
	Cancion[1].Dir = "recursos/waves.mp3";
	Cancion[2].Nombre = "Inicio 2";
	Cancion[2].Dir = "recursos/wavesbirds.mp3";
	Cancion[3].Nombre = "Inicio 3";
	Cancion[3].Dir = "recursos/Kirby";
	int indexCancion = 0;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR pScmdline, int iCmdshow)
{
	...
	m_applicationName = L"Animal Crossing AR";
	...
	while (!done)
	{
		...
		else
		{
			...
			if (m_Input->GetKeyDown(KeyCode.Seven)) {
				ReproductorCambiarCancionYReproduce(0);
			}
			if (m_Input->GetKeyDown(KeyCode.Eight)) {
				ReproductorCambiarCancionYReproduce(1);
			}
			if (m_Input->GetKeyDown(KeyCode.Nine)) {
