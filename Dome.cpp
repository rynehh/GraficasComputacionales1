...
Dome::Dome(string pathTexture, string pathTexture2, string pathTexture3, OpenGLClass* OpenGL, TextureClass* LoaderTextureRef, float radius)
{
	this->myMatrix = 0;
	Dome::radius = 10.0f;
	Dome::slices = 36;
	...
	final = 1;
	this->pathTexture = pathTexture;
	this->pathTexture2 = pathTexture2;
	this->pathTexture3 = pathTexture3;
	this->OpenGL = OpenGL;
	this->LoaderTexRef = LoaderTextureRef;
	this->radius = radius;
}
...
bool Dome::Initialize()
{
	...
	if (pathTexture2 != "") {
		TextureID2 = LoaderTexRef->Initialize(pathTexture2, true);
	}
	if (pathTexture3 != "") {
		TextureID3 = LoaderTexRef->Initialize(pathTexture3, true);
	}
...
bool Dome::Render(float* viewMatrix, float* projectionMatrix, float time, int opcion)
{
	// ** Definimos la escala **
	MatrixScale();
	...
	SkyShader->Pon1Entero((char*)"shaderTexture2", TextureID2);
	SkyShader->Pon1Entero((char*)"shaderTexture3", TextureID3);
	float* Time = new float[3] {time,time, time };
	SkyShader->PonVec3((char*)"lightDirection", Time);

	SkyShader->PonVec3((char*)"colorShader", DeltaColor);
	SkyShader->Pon1Entero((char*)"opcion", opcion);
	//Render shader
