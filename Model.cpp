...
bool Model::Initialize(const char* objPath, string texturePath) {
	...
	// Inicializaci�n de nuestra Bounding Box
	box = new BoundingBox();
	if (!box) {
		return false;
	}
...
bool Model::InitializeBuffers(OpenGLClass* OpenGL) {
	...
	//m_vertexCount * sizeof(VertexType) tama�o en bytes del buffer
	//vertices nuestro arreglo de vertices (posicion,normal,uv,tangent,binormal)
	/*
	GL_STATIC_DRAW 
	Especifica el patron que se usar� el buffer
	...
	*/
	...
	//glVertexAttribPointer( 0  indice del atributo,  3 cantidad de componentes (3 float), false no se normaliza, sizeof(VertexType) tama�o del vertice, 0 es nuestro puntero )
...
//Abre y rectifica que el modelo sea correcto y no este da�ado
